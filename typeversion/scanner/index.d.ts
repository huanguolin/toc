...
import { ScanError, ScanResult } from "./ScanResult";
...
type ScanOperator<S extends string> =
    S extends `${infer C extends Operators}${infer R extends string}`
        ? ScanResult<BuildToken<Safe<C, Operators>, C>, SkipSpace<R>>
...
type ScanNumber<S extends string, N extends NumStr | '' = ''> =
    S extends `${infer C extends NumChars}${infer R extends string}`
        ...
        : N extends ''
            ...
            : ScanResult<BuildToken<'number', N>, SkipSpace<S>>;

export type Scan<S extends string, A extends any[] = []> =
    S extends ''
        ? Push<A, EOF>
        : ScanNumber<S> extends ScanResult<infer T, infer R>
            ? Scan<R, Push<A, T>>
            : ScanOperator<S> extends ScanResult<infer T, infer R>
                ? Scan<R, Push<A, T>>
